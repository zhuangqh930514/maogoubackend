...
public class AiSampleSnapshotServiceImpl implements AiSampleSnapshotService {
    ...
    @Override
    public AiSampleV2 createOrGetSnapshot(SnapshotCommand command) {
        ...
        AiSampleV2 existing = findSample(command.userId(), stockCode, command.asOfTime(), samplePhase, universeVersion);
        if (existing != null) {
            return existing;
        }
        ...
        try {
            ...
            AiSampleV2 concurrent = findSample(command.userId(), stockCode, command.asOfTime(), samplePhase, universeVersion);
            if (concurrent != null) {
                return concurrent;
            }
            throw ex;
    ...
    @Override
    public List<AiSampleV2> findBatchSnapshots(Long userId, Long batchId, LocalDate tradeDate) {
        require(userId, "userId");
        require(batchId, "batchId");
        Objects.requireNonNull(tradeDate, "tradeDate");
        return List.copyOf(sampleMapper.selectList(new QueryWrapper<AiSampleV2>()
                .eq("user_id", userId)
    ...
    private AiSampleV2 findSample(Long userId, String stockCode, LocalDateTime asOfTime, String samplePhase, String universeVersion) {
        return sampleMapper.selectOne(new QueryWrapper<AiSampleV2>()
                .eq("user_id", userId)
