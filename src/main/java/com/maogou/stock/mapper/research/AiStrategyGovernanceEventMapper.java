...
public interface AiStrategyGovernanceEventMapper extends BaseMapper<AiStrategyGovernanceEvent> {
    ...
    @Insert("""
            INSERT INTO ai_strategy_governance_event (
                strategy_release_id, previous_champion_release_id, walk_forward_run_id,
                backtest_run_id, shadow_evaluation_id, event_key, event_type, decision_status,
                policy_version, actor_type, actor_user_id, reason, threshold_snapshot_json,
                evidence_json, occurred_at, created_at
            ) VALUES (
                #{item.strategyReleaseId}, #{item.previousChampionReleaseId},
    ...
    @Select("""
            ...
            FOR SHARE
            """)
    AiStrategyGovernanceEvent selectByEventKeyForShare(@Param("eventKey") String eventKey);
}
