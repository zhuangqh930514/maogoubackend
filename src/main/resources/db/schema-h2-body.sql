...
CREATE TABLE IF NOT EXISTS user_position_snapshot (
    ...
    KEY idx_user_position_snapshot_user_status_value (user_id, calculation_status, market_value),
    KEY idx_user_position_snapshot_user_updated (user_id, updated_at)
);
...
CREATE TABLE IF NOT EXISTS ai_research_universe (
    ...
    selection_policy_json CLOB NOT NULL,
...
CREATE TABLE IF NOT EXISTS ai_factor_definition (
    ...
    required_fields_json CLOB NOT NULL,
...
CREATE TABLE IF NOT EXISTS ai_training_dataset (
    ...
    source_query_json CLOB NOT NULL,
    selection_policy_json CLOB NOT NULL,
...
CREATE TABLE IF NOT EXISTS ai_strategy_release (
    ...
    active_guard TINYINT NULL,
    config_json CLOB NOT NULL,
    factor_snapshot_json CLOB NULL,
    validation_metrics_json CLOB NULL,
...
CREATE TABLE IF NOT EXISTS ai_prediction_evaluation (
    ...
    KEY idx_evaluation_version_prediction (evaluation_version, prediction_id),
...
CREATE TABLE IF NOT EXISTS ai_walk_forward_run (
    ...
    embargo_trading_days INT NOT NULL DEFAULT 5,
    status VARCHAR(32) NOT NULL DEFAULT 'PENDING',
    aggregate_metrics_json CLOB NULL,
...
CREATE TABLE IF NOT EXISTS ai_pipeline_issue (
    ...
    source_as_of TIMESTAMP NULL,
    attempt_no INT NOT NULL DEFAULT 0,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (user_id, report_id, pipeline_run_id, step_key, stock_code, provider_code, reason_code, attempt_no)
);
...
CREATE TABLE IF NOT EXISTS ai_strategy_governance_event (
    ...
    threshold_snapshot_json CLOB NOT NULL,
    evidence_json CLOB NOT NULL,
...
CREATE TABLE IF NOT EXISTS ai_user_strategy_binding (
    ...
    is_current TINYINT NOT NULL DEFAULT 1,
    current_guard TINYINT NULL,
    personalized_config_json CLOB NULL,
    seed_version VARCHAR(64) NULL,
...
CREATE TABLE IF NOT EXISTS ai_decision_policy_release (
    ...
    shadow_started_at TIMESTAMP NULL,
    ...
    UNIQUE (policy_key, active_guard)
);
...
CREATE TABLE IF NOT EXISTS ai_decision_policy_shadow_item (
    ...
    evaluation_status VARCHAR(32) NOT NULL DEFAULT 'PENDING_LABEL',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (user_id, trade_date, sample_id, shadow_policy_version, input_fingerprint)
);
...
CREATE TABLE IF NOT EXISTS ai_decision_policy_evaluation (
    ...
    comparison_json CLOB NOT NULL,
    decision_status VARCHAR(32) NOT NULL DEFAULT 'OBSERVING',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (policy_key, shadow_policy_version, window_start_date, window_end_date)
);
...
CREATE TABLE IF NOT EXISTS ai_daily_decision_item (
    ...
    trigger_factors_json CLOB NULL,
...
INSERT INTO ai_strategy_release
    (id, research_universe_id, model_family, version_no, title, model_version_id,
     status, release_role, active_guard, config_json, factor_snapshot_json, validation_metrics_json,
...
INSERT INTO ai_user_strategy_binding
    (user_id, strategy_release_id, model_family, is_current, current_guard,
     personalized_config_json, seed_version, bound_at)
SELECT
    id, 1, 'A_SHARE_MULTI_HORIZON', 1, 1,
...
CREATE INDEX IF NOT EXISTS idx_ai_user_notification_recent
    ON ai_user_notification (user_id, is_read, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ai_user_notification_report
    ON ai_user_notification (user_id, report_id);
...
CREATE TABLE IF NOT EXISTS ai_research_user_feedback (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (user_id, report_id, stock_code),
    FOREIGN KEY (user_id) REFERENCES user_account (id),
    FOREIGN KEY (report_id) REFERENCES ai_research_daily_report (id)
);

CREATE INDEX IF NOT EXISTS idx_ai_research_feedback_report
    ON ai_research_user_feedback (user_id, report_id, updated_at DESC);
...
CREATE TABLE IF NOT EXISTS ai_conditional_rule_experiment (
    ...
    threshold_snapshot_json CLOB NOT NULL,
    aggregate_metrics_json CLOB NOT NULL,
    input_fingerprint VARCHAR(128) NOT NULL,
    evaluated_at DATETIME(3) NOT NULL,
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
...
CREATE INDEX IF NOT EXISTS idx_conditional_rule_experiment_config
    ON ai_conditional_rule_experiment (user_id, trade_rule_config_id, candidate_status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_conditional_rule_experiment_window
    ON ai_conditional_rule_experiment (user_id, horizon_days, window_end_date DESC);
...
CREATE INDEX IF NOT EXISTS idx_conditional_rule_fold_test
    ON ai_conditional_rule_experiment_fold (experiment_id, test_start_date, test_end_date);
...
CREATE TABLE IF NOT EXISTS ai_conditional_rule_experiment_item (
    ...
    evidence_json CLOB NOT NULL,
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    UNIQUE (experiment_id, experiment_fold_id, sample_id, horizon_days),
...
CREATE INDEX IF NOT EXISTS idx_conditional_rule_experiment_item_test
    ON ai_conditional_rule_experiment_item (experiment_id, evaluation_partition, trade_date, triggered);
...
CREATE INDEX IF NOT EXISTS idx_conditional_rule_shadow_candidate
    ON ai_conditional_rule_shadow_observation (user_id, candidate_trade_rule_config_id, status, window_end_date DESC);
...
CREATE TABLE IF NOT EXISTS ai_conditional_rule_shadow_item (
    ...
    evidence_json CLOB NOT NULL,
    created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    UNIQUE (shadow_observation_id, sample_id, horizon_days),
...
CREATE INDEX IF NOT EXISTS idx_conditional_rule_shadow_item_triggered
    ON ai_conditional_rule_shadow_item (shadow_observation_id, candidate_triggered, baseline_triggered);
